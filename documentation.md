...
- **`validar_output.py`:** script aparte (no lo importa `main.py`) que corre los mismos checks manuales de la Fase 5 sobre un `output.json` ya generado: ≥30 procesos, slugs únicos, fechas en `YYYY-MM-DD`, sin strings vacíos donde el contrato pide `null`, URLs con forma válida. Reconstruye cada registro como `Proceso` (pydantic) antes de validar — eso ya garantiza que estén los 7 campos con el tipo correcto, así que `validar()` solo chequea las reglas que el tipo por sí solo no puede expresar (formato exacto de fecha, forma de URL, duplicados, mínimo). A propósito **no hace requests de red**: valida la forma de las URLs, no que respondan 200 (esa verificación puntual ya se hizo a mano en Fase 5 y quedó documentada) — mantiene el mismo principio que el resto del proyecto de no depender de la red donde no hace falta. Probado deliberadamente con un `output.json` corrompido a propósito (slug duplicado, fecha en formato crudo, string vacío en vez de `null`) para confirmar que detecta los 3 problemas y sale con código de salida 1.
